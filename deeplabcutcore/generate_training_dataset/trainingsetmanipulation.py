...
def create_training_dataset(
    ...
    )  # if the model does not exist >> throws error!
    ...
    if trainIndexes is None and testIndexes is None:
        splits = [
            (trainFraction, shuffle, SplitTrials(range(len(Data.index)), trainFraction))
        ...
        if len(trainIndexes) > 0:
            if userfeedback:
                trainposeconfigfile, _, _ = training.return_train_network_path(
                    config, shuffle=shuffle, trainFraction=trainFraction
                )
            ...
            path_train_config = str(
                os.path.join(
                    cfg["project_path"], Path(modelfoldername), "train", "pose_cfg.yaml"
                )
            )
